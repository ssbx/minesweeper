type question_mark_t = { from_hint : int * int; num_mines : int }
...
type grid_t = { g : cell_t array array; h : int; w : int }
...
let pretty_print ?(chan = stdout) grid =
  iter_ext
    ...
    (fun v ->
      ...
      | { content = Hint; exposed = false; hint = h; _ } ->
          Printf.fprintf chan "(%i) " h
      | { content = Hint; exposed = true; hint = h; _ } ->
          Printf.fprintf chan " %i  " h)
    grid.g
...
let neighbors x y w h =
  ...
  List.filter_map
    (fun (x_rel, y_rel) ->
      let nx = x + x_rel and ny = y + y_rel in
      ...
      grid.g.(y2).(x2) <-
        {
          v with
          qmarks =
            { from_hint = (from_x, from_y); num_mines = nmines } :: v.qmarks;
        })
    cells
...
let gen_mines_groups2 uhint grid =
  List.iter
    (fun (x, y) ->
      let unex = get_unexposed x y grid and cell = grid.g.(y).(x) in
