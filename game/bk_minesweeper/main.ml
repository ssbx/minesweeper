...
let data () =
  match !mines_data with
  | None -> failwith "noinit"
  | Some t -> t
;;
...
let get_neighbors at_x at_y =
  ...
  let c1 =
    List.fold_left
      (fun acc mx -> List.append [ mx, at_y - 1; mx, at_y; mx, at_y + 1 ] acc)
      []
      [ at_x - 1; at_x; at_x + 1 ]
  in
  ...
  let c2 =
    List.filter
      (fun (nx, ny) -> (nx = -1 || ny = -1 || nx = ncells_x || ny = ncells_y) = false)
      c1
  in
...
let gen_game_texture () =
  let d = data () in
  let text = d.texture_board
  and board = d.cells
  and rdr = d.renderer in
  ...
  in
  Array.iteri (fun x line -> Array.iteri (fun y cell -> iter_fun x y cell) line) board;
...
let gen_game () =
  let board = (data ()).cells
  and flaggs = (data ()).cells_flagged in
  Array.iteri (fun x l -> Array.iteri (fun y _ -> board.(x).(y) <- Empty false) l) board;
  Array.iteri (fun x l -> Array.iteri (fun y _ -> flaggs.(x).(y) <- false) l) flaggs;
  let coords =
    List.flatten (List.init ncells_x (fun x -> List.init ncells_y (fun y -> x, y)))
  in
...
let cell_from_cursor (e : Sdl.MouseButtonEvent.t) =
  ...
  | true ->
    ...
    (match cell_x < ncells_x && cell_y < ncells_y with
     | false ->
       Printf.printf
         "cursor scr:%i scr_cell:%i cursorx:%i cursory:%i cell_x:%i cell_y:%i"
         scr_w
         cw
         cur_x
         cur_y
...
let game_loosed bombs =
  ...
  List.iter
    (fun (x, y) -> draw_cell_at x y ~color:(255, 0, 0, 255) ~image:img_id_exploded rdr)
    bombs;
  Sdl.set_renderer_target rdr None
;;
...
let handle_mouse_button_down (e : Sdl.MouseButtonEvent.t) =
  ...
  then (
    ...
    | None -> ()
    | Some v ->
      if e.mb_button = 1 then click_left v else if e.mb_button = 3 then click_right v)
;;

let update_size w h =
  let d = data () in
  if Float.of_int w /. Float.of_int h < text_ratio
  then (
...
let handle_window_event = function
  ...
  | Sdl.WindowEventID.SDL_WINDOWEVENT_SIZE_CHANGED e -> update_size e.x e.y
  | _ ->
    let d = data () in
...
let create_texture rdr (r, g, b, a) =
  let w = ncells_x * cell_width
  and h = ncells_y * cell_width in
  let texture =
    Sdl.render_create_texture rdr Sdl.PixelFormat.RGBA8888 Sdl.TextureAccess.Target w h
  in
...
let on_update () =
  (*
     ...
     | true -> _anim_disco ()
     | false -> ());*)
  if !quit then Gdq.Exit else Gdq.Continue
;;
...
let on_draw () =
  ...
  if !boom = false then Sdl.render_copy rend ~texture:d.texture_shade ~dst_rect:rect ();
...
let () =
  ...
  Gdq.start ~win_name:"Démineur" ~on_ready ~on_event ~on_update ~on_draw ~on_close;
