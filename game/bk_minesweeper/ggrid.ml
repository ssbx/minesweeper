...
let pretty_print ?(chan = stdout) grid =
  iter_ext
    ...
    (fun v ->
       ...
       | { content = Hint; exposed = false; hint = h } -> Printf.fprintf chan "(%i) " h
       | { content = Hint; exposed = true; hint = h } -> Printf.fprintf chan " %i  " h)
    grid
;;
...
let neighbors x y w h =
  ...
  in
  List.filter
    (fun (nx, ny) -> (nx <> x || ny <> y) && nx < w && ny < h && nx >= 0 && ny >= 0)
    ns
;;
       ...
       | { content = Mine } ->
         ...
         List.iter
           (fun (nx, ny) ->
              ...
              | { content = Hint } as h -> grid.(ny).(nx) <- { h with hint = h.hint + 1 })
           neighs
       | _ -> ())
    grid
;;
         ...
         then (
           ...
           List.iter
             (fun (x, y) ->
                ...
                | { content = Mine } -> ()
                | { content = Hint } as v -> grid.(y).(x) <- { v with exposed = true }
                | { content = Empty } as v -> grid.(y).(x) <- { v with exposed = true })
             n3)
       | _ -> ())
    coords
;;
    ...
    Array.fold_left
      (fun acc1 l ->
         Array.fold_left
           (fun acc c ->
              match c with
              | { content = Hint; exposed = true } | { content = Empty; exposed = true }
                ->
                { acc with
                  ncells = acc.ncells + 1
...
let rec resolve cond grid w h =
  ...
  | { cells_exposed = exp; mines_flagged = flagged } as v
    when exp = cond.cells_exposed && flagged = cond.mines_flagged -> v
