...
let gencol i = function
  | b, Up when b > 255 - col_move -> b - (Random.int col_move |> set_intensity i)
  | b, Up -> b + (Random.int col_move |> set_intensity i)
  | b, Down when b < col_move -> b + (Random.int col_move |> set_intensity i)
  | b, Down -> b - (Random.int col_move |> set_intensity i)
;;
...
let rec update ms = function
  | [] -> ()
  | ({ transform_cmp = Some trans; mousein_cmp = None; color_rect = Some colr; _ } :
      Entity.t)
    :: tail ->
    apply trans colr;
    update ms tail
