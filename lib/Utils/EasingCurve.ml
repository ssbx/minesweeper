...
let circular_inout f =
  if f < 0.5 then 0.5 *. (1. -. sqrt (1. -. (4. *. (f *. f))))
...
let exponential_inout f =
  if f = 0.0 || f = 1.0 then f
  else if f < 0.5 then 0.5 *. Float.pow 2. ((20. *. f) -. 10.)
...
let bounce_inout f =
  if f < 0.5 then 0.5 *. bounce_in (f *. 2.)
